...
const DoctorDashboard = () => {
      ...
      const fetchData = async () => {
        try {
          const [appointmentsRes, profileRes] = await Promise.all([
            axios.get('http://localhost:8000/api/doctor/appointments/', {
  ...
  address_line1 = '',
  city = '',
  profile_picture = '/assets/placeholder.jpg',
} = profile;

// ✅ Combine address here
const formattedAddress = address_line1 && city
  ? `${address_line1}, ${city}`
    ...
    >
      ...
      >
        <motion.div variants={childVariants} style={{ textAlign: 'center', marginBottom: '20px' }}>
          <motion.img
            src={profile_picture.startsWith('/media/') ? `http://localhost:8000${profile_picture}` : profile_picture}
            alt="Profile Picture"
          ...
          >
            Welcome, Dr. {username}{' '}
            <span style={{ color: '#4b5563', fontSize: '1rem' }}>(Doctor)</span>
          </motion.h2>
        </motion.div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ backgroundColor: '#f3f4f6', textAlign: 'left' }}>
                  <th style={{ padding: '10px', borderBottom: '1px solid #d1d5db' }}>Patient</th>
